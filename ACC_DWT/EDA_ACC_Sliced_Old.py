...
def plot_all_categories():
    ...
    for i in range(min(5, len(df))):
        ...
        # # Highlight merged acc_intervals in PPG (if 'ppg' is present) using filtered index
        # if 'ppg' in plot_columns:
        #     ppg_index = plot_columns.index('ppg')
        #     plt.subplot(len(plot_columns), 1, ppg_index + 1)
