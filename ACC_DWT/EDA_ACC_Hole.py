    ...
    for i in range(min(5, len(df))):
        ...
        for j, col in enumerate(columns):
            ...
            if col == 'ppg':
                ...
                plot_data = [v for d in normalized_row for v in (d, d)]
                ...
                sqi_val = nk.ppg_quality(
                    normalized_row_np,
                    ppg_pw_peaks=nk.ppg_findpeaks(normalized_row_np, sampling_rate=50)["PPG_Peaks"],
                    sampling_rate=50,
                    method="templatematch"
                ).mean()
                print(f"SQI for Record {i+1}: {sqi_val}")
            else:
                plot_data = [v for d in data_values for v in (d, d,d, d)]
